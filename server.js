...
function sendMessageToAll(jsonParsedMessage) {
    // We are sending the current data to all connected clients
    Object.keys(clients).map((client) => {
        clients[client].sendUTF(jsonParsedMessage.utf8Data);
    });
}
    ...
    connection.on('message', function (message) {
        ...
        if (jsonParsedMessage.type === 'utf8') {
            ...
            if (jsonParsedMessage.broadcast === "user") {
                sendMessageToUser(jsonParsedMessage, jsonParsedMessage.to);

            } else if (jsonParsedMessage.broadcast === "all") {
                sendMessageToAll(jsonParsedMessage);
            }
        }
